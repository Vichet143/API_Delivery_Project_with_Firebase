...
interface Payment {
  ...
  paid_at: admin.firestore.FieldValue | admin.firestore.Timestamp | null;
  qr_code: string;
  qr_string: string;
  qr_md5: string;
...
async function qrStringToBase64(qrString: string): Promise<string> {
  return QRCode.toDataURL(qrString, { width: 400, margin: 2 });
}

function getExpiration(minutes = 15): string {
  return new Date(Date.now() + minutes * 60 * 1000).toISOString();
}

function startPolling(md5: string, paymentId: string): void {
  if (!BAKONG_TOKEN) {
  ...
  const interval = setInterval(async () => {
    try {
      const response = await fetch(
        `${BAKONG_API_BASE}/check_transaction_by_md5`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        console.error(
          `[startPolling] Bakong API error: ${response.status} ${response.statusText}`,
        );
        return;
      }
      ...
      if (result.responseCode === 0 && result.data) {
        console.log(
          `[startPolling] Payment ${paymentId} confirmed!`,
          result.data,
        );
        ...
        if (docSnap.exists) {
          ...
          await docRef.update({
            ...
            paid_at: admin.firestore.Timestamp.fromDate(
              new Date(result.data.acknowledgedDateMs),
            ),
          });
          ...
          if (deliveryId) {
            ...
            if (deliverySnap.exists) {
              await deliveryRef.update({
                paymentStatus: "paid",
                paymentAt: admin.firestore.Timestamp.fromDate(
                  new Date(result.data.acknowledgedDateMs),
                ),
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
              });
              console.log(
                `[startPolling] Updated delivery ${deliveryId} paymentStatus to paid.`,
              );
            } else {
              console.warn(
                `[startPolling] Delivery ${deliveryId} not found. Cannot update paymentStatus.`,
              );
            }
          }
        } else {
          console.error(
            `[startPolling] Payment ${paymentId} document not found. Cannot update.`,
          );
        }
      }
    } catch (err: any) {
  ...
  setTimeout(
    async () => {
      clearInterval(interval);
      console.log(
        `[startPolling] Polling stopped for payment ${paymentId} — QR expired.`,
      );
      ...
      if (docSnap.exists) {
        ...
        console.error(
          `[startPolling] Payment ${paymentId} document not found. Cannot mark as expired.`,
        );
      }
    },
    15 * 60 * 1000,
...
export async function createPayment(input: CreatePaymentInput): Promise<{
  ...
  const description =
    input.description ?? `Payment for delivery ${input.delivery_id}`;
  ...
  if (!individual || !individual.data || !individual.data.qr) {
    ...
    console.error(
      "[createPayment] Individual response:",
    ...
    throw new Error(
      `Failed to generate KHQR code. Response: ${JSON.stringify(individual)}`,
    );
  }

  // Verify QR contains correct currency (840=USD) and amount
  const qrString: string = individual.data.qr;
  if (!qrString.includes("5303840")) {
    console.warn(
      "[createPayment] WARNING: QR does not contain USD (5303840). Got:",
      qrString,
    );
  }
  if (!qrString.includes("5405")) {
    console.warn(
      "[createPayment] WARNING: QR does not contain amount field (5405). Got:",
      qrString,
    );
  }

  const qrMd5: string = individual.data.md5;
  ...
  console.log(
    `[createPayment] Created payment ${docRef.id} for delivery ${input.delivery_id} | amount: ${amount} USD`,
  );
...
export async function verifyPayment(input: VerifyPaymentInput): Promise<{
  ...
  if (payment.status === "paid" || payment.status === "failed") {
    return {
      ...
      paid_at: payment.paid_at
        ? (payment.paid_at as admin.firestore.Timestamp).toDate().toISOString()
        : null,
  ...
  if (new Date(payment.qr_expiration) < new Date()) {
    try {
      ...
      console.error(
        `[verifyPayment] Failed to update payment ${input.payment_id} to expired:`,
        error.message,
      );
    }
    return {
  ...
  const result: BakongCheckResponse = await response.json();
    ...
    try {
      ...
      if (deliveryId) {
        ...
        if (deliverySnap.exists) {
          ...
          console.log(
            `[verifyPayment] Updated delivery ${deliveryId} paymentStatus to paid.`,
          );
        } else {
          console.warn(
            `[verifyPayment] Delivery ${deliveryId} not found. Cannot update paymentStatus.`,
          );
        }
      }
    } catch (error: any) {
      console.error(
        `[verifyPayment] Failed to update payment ${input.payment_id} to paid:`,
        error.message,
      );
    }

    console.log(
      `[verifyPayment] Payment ${input.payment_id} confirmed as PAID.`,
    );
